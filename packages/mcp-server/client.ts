...
async function main() {
  ...
  try {
    ...
    try {
      const info = await client.callTool({ name: "server_info", arguments: {} as any });
    ...
    while (Date.now() < deadline) {
      try {
        const status = await client.callTool({ name: "bridge_status", arguments: {} as any });
        const txt = (status as any)?.content?.[0]?.text ?? String(status ?? "");
        if (/connected:\s*true/i.test(txt)) { connected = true; break; }
      } catch {}
      await sleep(500);
    }
    ...
    const args = { query: "site:example.com example", numResults: 3 } as const;
    try {
      const result = await client.callTool({ name: "search", arguments: args as any });
      console.log("[client] callTool result:\n" + JSON.stringify(result, null, 2));

      // If we have at least one URL, do a deep dive with visit_tool
      const firstUrl = (result as any)?._meta?.urls?.[0];
      if (typeof firstUrl === "string" && /^https?:\/\//i.test(firstUrl)) {
        console.log(`[client] visiting first URL with visit_tool: ${firstUrl}`);
        const visitRes = await client.callTool({ name: "visit_tool", arguments: { url: firstUrl, mode: "text" } as any });
      ...
      if (err && typeof err === "object") {
        ...
        for (const k of ["code", "data", "stack"]) {
          if (k in err) (extra as any)[k] = (err as any)[k];
        }
